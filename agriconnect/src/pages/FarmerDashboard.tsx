...
import { TrendingUp, Package, ShoppingBag, Star, ArrowRight } from 'lucide-react'
...
const StatCard = ({ icon, label, value, sub, color, delay }: any) => (
  ...
  >
    <Card padding="lg" className="relative overflow-hidden">
      <div className={`absolute top-0 right-0 w-24 h-24 rounded-full opacity-10 -translate-y-1/3 translate-x-1/3 ${color}`} />
...
export default function FarmerDashboard() {
  ...
  return (
    <>
      <Helmet><title>Tableau de bord – AgriConnect</title></Helmet>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-stone-500 dark:text-stone-400 mt-1">
            {user?.farmer_profile?.farm_name && `${user.farmer_profile.farm_name} · `}
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-6">
          ...
          >
            <Card padding="lg" className="h-full">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-semibold text-stone-800 dark:text-stone-100">Évolution des revenus</h2>
                <span className="text-xs text-stone-400 bg-stone-50 dark:bg-stone-800 px-3 py-1 rounded-full">6 derniers mois</span>
              </div>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={revenueData} margin={{ top: 5, right: 5, bottom: 0, left: 0 }}>
                  ...
                  />
                  <Area type="monotone" dataKey="revenus" stroke="#16a34a" strokeWidth={2.5} fill="url(#colorRev)" animationDuration={1500} />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
          </motion.div>
          ...
          >
            <Card padding="lg" className="h-full">
              <h2 className="font-semibold text-stone-800 dark:text-stone-100 mb-4">Actions rapides</h2>
              <div className="space-y-3">
                {[
                  { label: 'Ajouter un produit', icon: '➕', to: '/my-products' },
                  { label: 'Voir les commandes', icon: '📦', to: '/farmer-orders' },
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-stone-700 dark:text-stone-300 flex-1 text-left">{a.label}</span>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }}>
          <Card padding="lg">
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-semibold text-stone-800 dark:text-stone-100">Commandes récentes</h2>
              <Button variant="ghost" size="sm" onClick={() => navigate('/farmer-orders')}>
                Voir tout →
              </Button>
            </div>
            {!orders?.length ? (
              <p className="text-stone-400 text-sm text-center py-6">Aucune commande pour le moment.</p>
            ) : (
              <div className="space-y-3">
                  ...
                  >
                    <div>
                      <p className="font-medium text-stone-800 dark:text-stone-100 text-sm">
                        Commande #{order.id} · <span className="text-stone-500 font-normal">{order.buyer_name}</span>
                      </p>
                      <p className="text-xs text-stone-400 mt-0.5">{formatDate(order.created_at)}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <StatusBadge status={order.status} />
                      <span className="font-semibold text-stone-800 dark:text-stone-100 text-sm">{formatPrice(order.total_price)}</span>
                    </div>
                  </div>
                ))}
              </div>
