...
export default function Profile() {
  ...
  const { register, handleSubmit, watch, reset, formState: { errors, isSubmitting } } = useForm<PwForm>({
  ...
  const onSubmitPw = async (data: PwForm) => {
    try {
      ...
      const msg = resp ? Object.values(resp).flat()[0] : 'Erreur lors du changement'
      toast.error(msg)
    }
  }
  ...
  return (
    <>
      <Helmet><title>Mon profil – AgriConnect</title></Helmet>
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex items-center gap-5 mb-8">
            ...
            <div>
              <h1 className="text-2xl font-bold text-stone-800 dark:text-stone-100">{user.full_name}</h1>
          ...
          {activeTab === 'info' && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <Card padding="lg">
                <h2 className="font-semibold text-stone-800 dark:text-stone-100 mb-5">Informations personnelles</h2>
                <div className="space-y-4">
                  {[
                    { icon: <Mail className="w-4 h-4" />, label: 'Email', value: user.email },
                    { icon: <Phone className="w-4 h-4" />, label: 'Téléphone', value: user.phone || '—' },
                    { icon: <MapPin className="w-4 h-4" />, label: 'Localisation', value: user.location || '—' },
                  ].map((field) => (
                    <div key={field.label} className="flex items-center gap-3 py-3 border-b border-stone-100 dark:border-stone-800 last:border-0">
                      <div className="w-8 h-8 rounded-lg bg-stone-50 dark:bg-stone-800 flex items-center justify-center text-stone-400">
                        {field.icon}
                      </div>
                      <div>
                        <p className="text-xs text-stone-400 mb-0.5">{field.label}</p>
                        <p className="text-sm font-medium text-stone-800 dark:text-stone-100">{field.value}</p>
                      </div>
                    </div>
                  ))}
                  ...
                  {user.farmer_profile && (
                    <div className="flex items-center gap-3 py-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-stone-800 dark:text-stone-100">{user.farmer_profile.farm_name}</p>
                      </div>
                    </div>
                  )}
                </div>
          ...
          {activeTab === 'security' && (
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
              <Card padding="lg">
                <h2 className="font-semibold text-stone-800 dark:text-stone-100 mb-5">Changer le mot de passe</h2>
