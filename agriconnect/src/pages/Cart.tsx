...
export default function Cart() {
  ...
  // Correction 1 : utiliser 'PICKUP_POINT' au lieu de 'PICKUP'
  const [deliveryType, setDeliveryType] = useState<'HOME_DELIVERY' | 'PICKUP_POINT'>('HOME_DELIVERY')

  const handleOrder = async () => {
    await createOrder.mutateAsync({
      delivery_type: deliveryType,
      delivery_address: '',   // Correction 2 : ajout du champ (vide pour retrait)
  ...
  return (
    <>
      <Helmet><title>Mon panier – AgriConnect</title></Helmet>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-8">
            <ShoppingCart className="w-7 h-7 text-green-700 dark:text-green-400" />
            <h1 className="text-3xl font-bold text-stone-800 dark:text-stone-100">Mon panier</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <Card padding="sm">
                      <div className="flex gap-4 items-center">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-stone-800 dark:text-stone-100 truncate">{item.product.name}</h3>
                          <p className="text-sm text-stone-500 dark:text-stone-400">{item.product.farmer_name}</p>
                          <p className="text-green-700 dark:text-green-400 font-semibold mt-1">{formatPrice(item.product.price)} / unité</p>
                        </div>
            ...
            <div className="lg:col-span-1">
              <Card padding="lg" className="sticky top-24">
                <h2 className="font-bold text-stone-800 dark:text-stone-100 text-lg mb-5">Résumé</h2>

                <div className="space-y-2 mb-5">
                  <p className="text-sm font-medium text-stone-700 dark:text-stone-300">Mode de livraison :</p>
                  {[
                ...
                <button onClick={clearCart} className="w-full mt-3 text-xs text-stone-400 hover:text-red-500 transition-colors">
                  Vider le panier
                </button>
              </Card>
            </div>
