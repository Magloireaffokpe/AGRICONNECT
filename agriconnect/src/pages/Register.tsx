...
const schema = z
  .object({
    ...
    phone: z
      .string()
      .min(8, "Numéro de téléphone invalide (8 chiffres minimum)"),
    location: z.string().min(2, "Ville / localisation requise"),
    role: z.enum(["FARMER", "BUYER"]),
    farm_name: z.string().optional(),
    bio: z.string().optional(), // 👈 ajout
    password: z
      .string()
...
export default function Register() {
  ...
  const {
    ...
    defaultValues: {
      role: (params.get("role") as "FARMER" | "BUYER") || "BUYER",
    },
    mode: "onChange",
  });
  ...
  const nextStep = async () => {
    ...
    if (!isValidStep) {
      const fieldErrors = Object.keys(errors)
        ...
        .join(", ");
      toast.error(
        fieldErrors || "Veuillez corriger les erreurs avant de continuer",
      );
      return;
    }
    setStep((s) => s + 1);
              ...
              <div key={s} className="flex items-center gap-2">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                    i < step
                      ...
                      : i === step
                        ? "bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 ring-2 ring-green-600"
                ...
                {i < STEPS.length - 1 && (
                  <div className="flex-1 h-0.5 w-8 bg-stone-200 dark:bg-stone-700">
                    <div
                      className={`h-full bg-green-600 transition-all duration-500 ${i < step ? "w-full" : "w-0"}`}
                    />
                  </div>
                )}
              </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 0 && (
              <motion.div
                ...
                className="space-y-4"
              >
                <p className="text-sm font-medium text-stone-700 dark:text-stone-300 mb-4">
                  Vous êtes :
                </p>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-semibold text-stone-800 dark:text-stone-100">
                          {r === "BUYER" ? "Acheteur" : "Agriculteur"}
                        </div>
                        <div className="text-xs text-stone-500 mt-0.5">
                          {r === "BUYER"
                            ? "Je cherche des produits frais locaux"
                            : "Je vends mes récoltes directement"}
                        </div>
                      </div>
                      {role === r && (
                        <div className="ml-auto w-5 h-5 rounded-full bg-green-600 flex items-center justify-center text-white text-xs">
                          ✓
                        </div>
                      )}
                    </div>
                ...
                {role === "FARMER" && (
                  <>
                    ...
                    <Input
                      ...
                      placeholder="Décrivez votre ferme, vos méthodes..."
